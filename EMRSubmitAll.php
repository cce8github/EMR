...
<div class="DoNotPrint" id="BottomButtons" style="position: absolute; top:50px; left:750px; background-color:transparent;">
<br><?php echo " <a href=\"EMRDirList2.php?\"><b>Select Another File</b></a></A>";?><br><br>
</div>
...
<!img id='BGImage' src="${oscar_image_path}Bedside_Flow_Sheet_Single_Page.JPG" border="3" style="position: absolute; left: 5px; height:775; top: 5px; width:900px">
...
<p><b><h3>Data Submitted Successfully... Thank You.</h3></b>
</div>
...
<?php 
//Generating the Hourly Data to a file
$userFile = "/home1/chikasit/public_html/myprojects/EMR/EMRData1.txt";
$fp = file($userFile);
$i=0; 
$data2 = "";
$params_all = "";
while($fp[$i]<>'') { 
   $contents = $fp[$i];    
   $data = explode("\t", $fp[$i]);
   $content = $data2."$data[0]"."<td>".$data[1]."<td>".$data[2]."<td>".$data[3]."<td>".$data[4]."<td>".$data[5]."<tr>"."<td>";
   echo $content;
...
$params_all = $params_all."\t".$query;

   $Dat = date(DATE_ATOM);
   $contentx = "$data[0]"."\t".$data[1]."\t".$data[2]."\t".$data[3]."\t".$data[4]."\t".trim($data[5])."\t".$fileData[1]."\t".$Dat."\n";
//echo $fileData[1];

   $i++;
}
//echo $params_all;
echo " <p> <a href=\"EMRFinal.php?varname=$params_all\">Proceed to Submit</a></A>";
?>
</td> </tr> </table>

<?php
//Generating and Saving the Data Values to a file
//TYPE "/home1/chikasit/public_html/myprojects/EMR/EMRData1.txt" >> "/home1/chikasit/public_html/myprojects/EMR/EMROutput.txt";
$content_data = file_get_contents('/home1/chikasit/public_html/myprojects/EMR/EMRData1.txt');
